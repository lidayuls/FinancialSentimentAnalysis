...
Description:
This script implements the **Financial Numeric Reasoning Expert (FinNum) (Llama3-FinanceMath) ** 
...
This script implements two main stages:
1.  **Pretraining on FinanceMath Dataset:** Continual pretraining of a base LLM
    (Llama-3-8b) on the FinanceMath dataset to enhance its numerical reasoning
    capabilities in financial contexts. This creates the `Llama3-FinanceMath` expert.
2.  **Fine-tuning on Financial Sentiment Datasets:** Fine-tuning the pretrained
    `Llama3-FinanceMath` model on multiple financial sentiment analysis datasets
    (FPB, FiQA-SA, TFNS) using instruction tuning. This adapts the numerically-
    enhanced model for the specific task of sentiment classification.


Key Components:
- Model: Llama-3-8b (4-bit quantized) -> Finetuned on FinanceMath -> Finetuned on Sentiment.
- Datasets: yale-nlp/FinanceMath (pretraining), FPB, FiQA-SA, TFNS (fine-tuning).
- Training: QLoRA (r=16) with 4-bit quantization.
- Output: Saved model and extracted last-layer hidden states for downstream tasks.
...
# ---  1. Continual pretraining of Llama-3-8b on the FinanceMath dataset to create Llama3-FinanceMath expert ---
...
financemath_dataset = load_dataset("yale-nlp/FinanceMath")

# Define prompt template for FinanceMath pretraining
alpaca_prompt = """### python_solution:
{}
### tables:
{}
### question_id:
...
def formatting_prompts_func(examples):
    python_solutions = examples["python_solution"]
    tables = examples["tables"]
    question_ids = examples["question_id"]
    questions = examples["question"]
    topics = examples["topic"]
    ground_truths = examples["ground_truth"]
    
    texts = []
    for python_solution, table, question_id, question, topic, ground_truth in zip(
        python_solutions, tables, question_ids, questions, topics, ground_truths
    ):
        text = alpaca_prompt.format(python_solution, table, question_id, question, topic, ground_truth) + EOS_TOKEN
        texts.append(text)
    return {"text": texts}

financemath_dataset = financemath_dataset.map(formatting_prompts_func, batched=True)

# Train on FinanceMath dataset
trainer = SFTTrainer(
    model=model,
    tokenizer=tokenizer,
    train_dataset=financemath_dataset["validation"],
...
print("Starting FinanceMath pretraining...")
model.train()
trainer_stats = trainer.train()

# Save pretrained model
trainer.save_model("outputs/Pretrained_Llama_on_FinanceMATH")
print("FinanceMath pretraining completed and model saved.")
...
print("Financial Numeric Reasoning Expert training completed successfully.")
...
# Generate save paths
model_name = "Pretrained_Llama_on_FinanceMATH"
